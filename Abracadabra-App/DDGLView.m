...
- (void)addParticle:(DDParticle *)particle {
	...
	if (![timer isValid]) {
		[timer release];	
        timer = [[NSTimer scheduledTimerWithTimeInterval:1.0f / 15.0f
                                                  target:self
...
- (void)setImage:(NSImage *)newImage{
    ...
    unsigned char *data = [[newImage bestRepresentationForDevice:nil] bitmapData];
	gluBuild2DMipmaps(GL_TEXTURE_2D, 4, [newImage size].width, [newImage size].height, GL_RGBA, GL_UNSIGNED_BYTE, data);
